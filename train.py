...
def evaluate():
    ...
    print("llr_img ",valid_lr_img.shape)
    print("hr_img ",valid_hr_img.shape)
    hr_size1 = [valid_lr_img.shape[0], valid_lr_img.shape[1]]
    print("dsfs  ",hr_size1)
    valid_lr_img= valid_lr_img.astype(np.float32)
    #valid_lr_img = cv2.resize(valid_lr_img,None,fx=0.25,fy=0.25,interpolation=cv2.INTER_AREA)
    valid_lr_img = cv2.resize(valid_lr_img, dsize=(hr_size1[1] // 4, hr_size1[0] // 4),interpolation=cv2.INTER_AREA)
    ...
    valid_hr_img= valid_hr_img.astype(np.float32)
